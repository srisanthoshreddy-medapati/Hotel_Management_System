		...
		while ($row=mysqli_fetch_row($result))
	   	{
			...
			{
				$room = $row[3];
				$sql1 = "SELECT * FROM rooms_count WHERE RoomType='$room'";
				$result1 = mysqli_query($conn,$sql1);
				$r = mysqli_fetch_row($result1);
				if($r[1]>0)
				{
					$r[1] = $r[1] - 1;
					$sql2 = "UPDATE rooms_count SET AvailableRooms='$r[1]' WHERE RoomType='$room'";
					mysqli_query($conn, $sql2);
					$r[2] = $r[2] + 1;
					$sql2 = "UPDATE rooms_count SET OccupiedRooms='$r[2]' WHERE RoomType='$room'";
					mysqli_query($conn, $sql2);
